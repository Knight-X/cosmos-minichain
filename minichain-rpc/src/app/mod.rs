...
use module_service::module_server::{Module as ModuleService, ModuleServer};
use tonic::transport::Server;
use module_service::module_client::ModuleClient;
use tonic::{Request, Response, Status};

use crate::app::module_service::{ModuleRequest, ModuleReply, ModuleQuery, ModuleResponseQuery};

pub mod module_service {
        tonic::include_proto!("moduleservice");
}
...
impl From<tendermint_proto::crypto::ProofOp> for module_service::ProofOps {
    fn from(item: tendermint_proto::crypto::ProofOp) -> Self {
        let mut _data = Vec::new();
        let g = module_service::ProofOp { r#type: item.r#type,
        key: item.key,
        data: item.data};
        _data.push(g);
        module_service::ProofOps { 
         ops: _data
        }
    }
}
...
impl<S: Default + ProvableStore + 'static> ModuleService for ModuleServices<S> {
        async fn init(
          ...
          let reply = crate::app::module_service::ModuleReply {
     ...
     async fn query(
        ...
        let _op = module_service::ProofOps::from(_m.proof.unwrap().pop().unwrap());
        let mut _data = Vec::new();
        _data.push(_m.data);
        let reply = crate::app::module_service::ModuleResponseQuery {
                   data: _data, 
                   proof_ops: Some(_op) 
